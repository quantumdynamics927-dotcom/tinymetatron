...
GATE_DEFS = [
    ...
    ("en_tag_atomic",           "python -c \"import sys; sys.path.insert(0,'experiments/english_first_tokenizer'); from train_en_tokenizer import evaluate_tokenizer, TEST_CASES; from tokenizers import Tokenizer; t=Tokenizer.from_file('experiments/english_first_tokenizer/tokenizers/tinymetatron_v2_en_8k/tokenizer.json'); enc=t.encode('<|en|>'); assert len(enc.ids)==1, f'<|en|> not atomic: {enc.ids}'\"",
                                 "<|en|> encodes as exactly 1 token"),
    ("unk_count_zero",           "python -c \"import sys; sys.path.insert(0,'experiments/english_first_tokenizer'); from train_en_tokenizer import evaluate_tokenizer; from tokenizers import Tokenizer; t=Tokenizer.from_file('experiments/english_first_tokenizer/tokenizers/tinymetatron_v2_en_8k/tokenizer.json'); _,s=evaluate_tokenizer(t); assert s['all_unk']==0, f'unk count={s[\\\"all_unk\\\"]}'\"",
                                 "Unknown token count = 0 on TEST_CASES"),
    ("round_trip",               "python -c \"import sys; sys.path.insert(0,'experiments/english_first_tokenizer'); from train_en_tokenizer import TEST_CASES; from tokenizers import Tokenizer; t=Tokenizer.from_file('experiments/english_first_tokenizer/tokenizers/tinymetatron_v2_en_8k/tokenizer.json'); errors=[]; [errors.append(n) for n,c in TEST_CASES.items() if t.decode(t.encode(c).ids, skip_special_tokens=False)!=c]; assert not errors, f'Round-trip failed: {errors}'\"",
